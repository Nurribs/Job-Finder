...
const JobDetail = () => {
  ...
  const [applicationStatus, setApplicationStatus] = useState('apply'); // Başlangıç durumu 'apply' olsun
  ...
  const getJobDetails = async () => {
    ...
    try {
      ...
      if (res?.data) {
        ...
        console.error("Job details could not be retrieved.");
      }
    } catch (error) {
  ...
  const handleApplyNowClick = async () => {
    ...
    try {
      if (window.confirm("İşi kabul ediyormusunuz?")) {
        const res = await apiRequest({
          url: "/jobs/kabul-job/" + job?._id,
          token: user?.token,
          method: "POST",
        });
        ...
        if (res?.success) {
          alert(res?.message);
          setApplicationStatus('submitted'); // Başvuru yapıldıktan sonra durumu değiştir
    ...
    <div className='container mx-auto'>
      <div className='w-full flex flex-col md:flex-row gap-10'>
          ...
          <div className='w-full h-fit md:w-2/3 2xl:2/4 bg-white px-5 py-10 md:px-10 shadow-md'>
            ...
            <div className='w-full flex flex-wrap md:flex-row gap-2 items-center justify-between my-10'>
              ...
              <div className='bg-[#fed0ab] w-40 h-16 px-6 rounded-lg flex flex-col items-center justify-center'>
                <span className='text-sm'>Başvuru Sahibi Sayısı</span>
                <p className='text-lg font-semibold text-gray-700'>
              ...
              <div className='bg-[#cecdff] w-40 h-16 px-6 rounded-lg flex flex-col items-center justify-center'>
                <span className='text-sm'>Boş Pozisyon Sayısı</span>
                <p className='text-lg font-semibold text-gray-700'>
                  {job?.vacancies}
                </p>
              </div>
            ...
            <div className='my-6'>
              {selected === "0" ? (
                <>
                  <p className='text-xl font-semibold'>İş Tanımı</p>

                  <span className='text-base'>{job?.detail[0]?.desc}</span>

                  {job?.detail[0]?.requirements && (
                    <>
                      <p className='text-xl font-semibold mt-8'>Gereklilik</p>
            ...
            <div className='w-full'>
              {user?._id === job?.company?._id ? (
                ...
                <CustomButton
            title={applicationStatus === 'apply' ? 'Apply Now' : 'Job Submitted'} // Metni duruma göre ayarla
            onClick={handleApplyNowClick}
            containerStyles={`w-full flex items-center justify-center text-white ${
              applicationStatus === 'apply' ? 'bg-black' : 'bg-red-700'
        ...
        <div className='w-full md:w-1/3 2xl:w-2/4 p-5 mt-20 md:mt-0'>
          <p className='text-gray-500 font-semibold'>Benzer İş İlanı</p>
